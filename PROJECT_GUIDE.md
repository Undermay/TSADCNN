...
TSADCNN (Time-Space Attention Deep Convolutional Neural Network) 是一个基于对比学习的轨迹关联深度学习项目。该项目专注于解决多目标跟踪中的轨迹关联问题，通过学习轨迹的时空特征表示来判断不同轨迹段是否属于同一目标。

### 核心特性
- **对比学习框架**：使用old-new轨迹对进行训练，学习区分同目标和不同目标的轨迹
- **时空特征融合**：结合LSTM时间建模和CNN空间建模
- **多种归一化策略**：支持段级、场景级和全局MinMax归一化
...
- **对比损失**：基于余弦相似度的配对损失，区分正负样本
- **对称约束**：基于相关矩阵的对称性约束，提高特征质量
...
- **P@K**: 在前K个最相似的轨迹中，正确关联的比例
- **AP**: 平均精度，综合考虑所有排序位置的精度
