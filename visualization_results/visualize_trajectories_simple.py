...
def plot_trajectory(ax, trajectory, color, linestyle='-', alpha=0.7, linewidth=1.5):
    """绘制单条轨迹"""
    if len(trajectory) < 2:
        return
    ...
    ax.scatter(trajectory[-1, 0], trajectory[-1, 1], 
              color=color, marker='^', s=40, alpha=0.8, 
              edgecolors='white', linewidth=1, zorder=5)

def plot_simple_trajectories(df, title, save_path, max_trajectories=200):
    """绘制简化版轨迹图"""
    print(f"🎨 开始绘制: {title}")
    ...
    if len(df) > max_trajectories:
        ...
        sample_df = df
        print(f"   使用全部 {len(sample_df)} 条轨迹")
    
    # 按目标分组绘制
    target_colors = {}
    color_idx = 0
    
    print(f"   开始绘制 {len(sample_df)} 条轨迹...")
    
    for idx, (_, row) in enumerate(sample_df.iterrows()):
        if idx % 50 == 0:
            print(f"   已处理 {idx}/{len(sample_df)} 条轨迹")
        ...
        try:
            ...
            print(f"   ⚠️ 轨迹 {idx} 绘制失败: {e}")
            continue
    
    print(f"   轨迹绘制完成，开始设置图形属性...")
    ...
    legend_elements = [
        ...
        plt.scatter([], [], color='gray', marker='o', s=30, label='起点'),
        plt.scatter([], [], color='gray', marker='^', s=40, label='终点')
    ]
...
def plot_scene_detail(df, scene_ids, title, save_path):
    """绘制场景详细轨迹"""
    print(f"🎨 开始绘制: {title}")
    ...
    for i, scene_id in enumerate(scene_ids):
        ...
        print(f"   绘制场景 {scene_id}, 共 {len(scene_data)} 条轨迹")
        ...
        for idx, row in scene_data.iterrows():
            try:
                ...
                print(f"   ⚠️ 场景 {scene_id} 轨迹绘制失败: {e}")
                continue
        
        # 设置子图属性
        ax.set_xlabel('X坐标 (米)', fontsize=10)
        ax.set_ylabel('Y坐标 (米)', fontsize=10)
        ax.set_title(f'场景 {scene_id}', fontsize=12, fontweight='bold')
        ax.grid(True, alpha=0.3)
        ax.set_aspect('equal', adjustable='box')
...
def main():
    ...
    try:
        ...
        try:
            ...
            good_scenes = scene_counts[scene_counts >= 5].index.tolist()  # 降低阈值
